...
export default function AgentMarket() {
  ...
  // 能力包详情弹窗（存包分组键，渲染时从 packageCards 实时反查以拿最新成员状态）与包级激活忙碌态
  ...
  // 包分组键：package_module（= module.json id / package.json name slug）优先，回退 module（legacy 手写 SKU 的模块目录名）。
  ...
  // 一个秘技包 = 一个卡片，包内多项能力（SKU）可被作为工具调用；包状态由成员 SKU 聚合。
  ...
  // 复用后端 ToggleAgentActive（内部含购买门禁 + 按部署拉起 runtime），与云端对称。
  ...
  return (
    <div className="pt-8 pb-16 px-4 max-w-6xl mx-auto min-h-screen">
      ...
      {/* 卡片网格：能力包卡 + 独立 SKU 卡（一个秘技包 = 一个卡片，包内多项能力可被作为工具调用） */}
      ...
      {/* 能力包详情弹窗：列出成员 SKU 逐条勾选（勾选=激活加入 tools）；包状态强关联成员模块 */}
      {packageDetail && (() => {
