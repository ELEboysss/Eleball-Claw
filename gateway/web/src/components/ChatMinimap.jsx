...
// Hook：为消息列表创建稳定 ref 数组
export function useMessageRefs(count) {
  const refs = useRef([])
  refs.current = Array.from({ length: count }, (_, i) => refs.current[i] ?? null)
  return refs
}

export default function ChatMinimap({ messages, streamingMessage, scrollContainer, messageRefs }) {
  ...
  // 节流 DOM 测量（消息变化/尺寸变化时触发，最多 150ms 一次）
  ...
  const scrollToMinimapRatio = useCallback((viewportTopRatio) => {
    const el = scrollContainer.current
    if (!el) return
  ...
  return (
    <div
      ...
      className="border-l border-eleball-outline-variant bg-eleball-surface"
    >
      {/* 视口指示器 */}
      <div
        style={{
        ...
        return (
          <div
            ...
            style={{
              ...
              marginRight: 6,
              background: 'white',
              borderTop: `1px solid ${isNearest ? color.border : '#e5e7eb'}`,
              borderRight: `1px solid ${isNearest ? color.border : '#e5e7eb'}`,
              borderBottom: `1px solid ${isNearest ? color.border : '#e5e7eb'}`,
