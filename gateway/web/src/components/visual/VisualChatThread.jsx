...
function TaskResultCard({ task, onSelect, onContinue }) {
  ...
  return (
    <div
      ...
      className="group cursor-pointer rounded-xl border border-eleball-vs-border-variant bg-eleball-vs-surface p-3 hover:border-eleball-primary/50 transition-colors max-w-[80%]"
    >
      {/* 状态与成本 */}
      <div className="mb-2 flex items-center justify-between text-xs">
        <span className={`font-medium ${STATUS_COLORS[task.status] || 'text-eleball-vs-text-muted'}`}>
          {STATUS_LABELS[task.status] || task.status}
        </span>
        <span className="text-eleball-vs-text-dim">消耗 {formatCost(task.cost, task.currency)}</span>
      </div>

      {/* 结果预览 */}
      {task.status === 'pending' || task.status === 'running' ? (
        <div className="flex items-center gap-2 text-sm text-eleball-vs-text-muted">
        ...
        <div className="text-sm text-eleball-vs-error">{task.error_message || '生成失败'}</div>
      ) : urls.length > 0 ? (
        <div className="space-y-2">
          <div className="rounded-lg overflow-hidden bg-eleball-vs-bg">
            {isVideo ? (
          ...
          {urls.length > 1 && (
            <div className="flex gap-2 overflow-x-auto">
              {urls.map((u, idx) => (
                <img key={idx} src={u} alt={`结果 ${idx + 1}`} className="w-16 h-16 object-cover rounded-lg border border-eleball-vs-border-variant" />
              ))}
            </div>
          )}
        </div>
      ) : (
        <div className="text-sm text-eleball-vs-text-dim">无可用结果</div>
      )}
      ...
      {canContinue && onContinue && (
        <button
          ...
          className="mt-2 flex items-center gap-1 text-xs text-eleball-vs-accent hover:text-white transition-colors"
        >
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center text-eleball-vs-text-dim">
        <div className="mb-3 rounded-full bg-eleball-vs-surface-variant p-4">
        ...
        return (
          <div key={task.id} className="flex flex-col gap-3">
            ...
            <div className="flex items-start gap-3 justify-end">
              <div className="max-w-[80%] rounded-2xl rounded-tr-sm bg-eleball-primary px-4 py-2.5 text-sm text-white">
                {task.prompt || '未填写提示词'}
              </div>
              <div className="flex-shrink-0 rounded-full bg-eleball-primary/20 p-1.5">
                <User className="w-4 h-4 text-eleball-vs-accent" />
              </div>
            </div>

            {/* 助手结果 */}
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0 rounded-full bg-eleball-vs-surface-variant p-1.5">
                <Bot className="w-4 h-4 text-eleball-vs-text-muted" />
              </div>
