...
//   - 当前仅支持 python（验收：无 python 机器一键装后 firecrawl 可跑）；node/npx 托管
//     下载暂未实现（结构留扩展位），locateCommand 对其仍返回 InterpreterMissingError。
...
// command 当前支持 python/python3/python3.12 等（统一按 python 处理）；node/npx 仅系统查找，
// 缺失时返回错误（暂不支持托管下载）。
...
// 供 locateCommand 在系统未找到时回退；node/npx 暂不支持托管，返回空。
