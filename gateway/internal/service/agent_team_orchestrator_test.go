...
func TestFormatCatalogBlock(t *testing.T) {
	...
	assert.Contains(t, block, "你可以委派任务的协作助手（经 CallAssistant 工具调用")
...
func TestCallAssistant_SubLoopDepthLimit(t *testing.T) {
	...
	fix.toolEnv.UsageAccumulator = func(u *llm.Usage) { accumulated = addUsage(accumulated, u) }
	...
	// 子 Usage 两次调用（8+4=12）全部经 accumulator 进账
	require.NotNil(t, accumulated)
	assert.Equal(t, 12, accumulated.TotalTokens)
	// 委派计数 +1
