...
func TestTeamMemory_InjectedIntoSystemPrompt(t *testing.T) {
	...
	msgs, _ := agentSvc.buildInitialMessages(ctx, AgentExecuteRequest{Message: "项目代号是什么"}, nil, teamMemoryTestUser, team.ID, "", false, false, model.PermissionModeDefault)
	...
	// 未分组对话（teamID 为空）：不注入
	msgs, _ = agentSvc.buildInitialMessages(ctx, AgentExecuteRequest{Message: "项目代号是什么"}, nil, teamMemoryTestUser, "", "", false, false, model.PermissionModeDefault)
	...
	msgs, _ = agentSvcBare.buildInitialMessages(ctx, AgentExecuteRequest{Message: "项目代号是什么"}, nil, teamMemoryTestUser, team.ID, "", false, false, model.PermissionModeDefault)
	systemContent, _ = msgs[0].Content.(string)
	assert.NotContains(t, systemContent, "组共享记忆")
}
